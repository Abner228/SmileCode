...
num_classes = 2
test_data_list = os.listdir('/data/zym/workspace/noisy/testset_resample_1.0X0.6X0.6')
test_dataset = BreastTumor(test_data_list, transform=Norm())
test_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False)

net = Unet(1, 2).cuda()
net.load_state_dict(torch.load('/data/zym/experiment/noisy/stage3/epoch_200.pth'))

pred_dir = '/data/zym/experiment/noisy/stage3/infer_test'
myMakedirs(pred_dir, overwrite=False)

for sampled_batch in tqdm(test_dataloader):
    net.eval()
    case, img = sampled_batch['case'], sampled_batch['image']
    ...
    pred_path = os.path.join(pred_dir, case[0] + '_pred.nii.gz')
